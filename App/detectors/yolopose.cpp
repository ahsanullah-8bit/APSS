...
YOLOPose::YOLOPose(const std::string &modelPath, const std::string &labelsPath) : YOLOInference(modelPath, labelsPath)
{
    Ort::AllocatedStringPtr kpt_shape = modelMetadata().LookupCustomMetadataMapAllocated("kpt_shape", allocator());
    qDebug() << "kpt_shape:" << kpt_shape.get();
}
...
void YOLOPose::drawPredictions(cv::Mat &image, const PredictionList &predictions, float maskAlpha) const
{
    ...
    Utils::drawPoseEstimation(image, predictions, skeleton);
}

void YOLOPose::drawPredictionsMask(cv::Mat &image, const PredictionList &predictions, float maskAlpha) const
{
    drawPredictions(image, predictions, maskAlpha);
}

MatList YOLOPose::cropPredictions(const cv::Mat &image, const PredictionList &predictions) const
{
    return cropPredictions(image, predictions, false);
}

MatList YOLOPose::cropPredictions(const cv::Mat &image, const PredictionList &predictions, bool perspectively) const
{
    MatList crops;
    for (const auto &prediction : predictions) {
        ...
        if (perspectively) {
            Utils::perspectiveCrop(image, result, prediction.points);
        } else {
            Utils::crop(image, result, prediction.box);
        }
        crops.emplace_back(result);
    }

    return crops;
}

void YOLOPose::drawPredictions(MatList &images, const std::vector<PredictionList> &predictionsList, float maskAlpha) const
{
    YOLOInference::drawPredictions(images, predictionsList, maskAlpha);
}

void YOLOPose::drawPredictionsMask(MatList &images, const std::vector<PredictionList> &predictionsList, float maskAlpha) const
{
    YOLOInference::drawPredictionsMask(images, predictionsList, maskAlpha);
}

std::vector<MatList> YOLOPose::cropPredictions(const MatList &images, const std::vector<PredictionList> &predictionsList) const
{
    return YOLOInference::cropPredictions(images, predictionsList);
}

std::vector<PredictionList> YOLOPose::postprocess(const MatList &originalImages,
