...
YOLODetection::YOLODetection(const std::string &modelPath, const std::string &labelsPath)
...
void YOLODetection::drawPredictionsMask(cv::Mat &image, const prediction_vec &predictions, float maskAlpha) const
{
    Utils::drawBoundingBoxMask(image, predictions, classNames(), classColors(), maskAlpha);
}

mat_vec YOLODetection::cropPredictions(const cv::Mat &image, const prediction_vec &predictions) const
{
    mat_vec results;
    results.reserve(predictions.size());
    for (const Prediction& prediction : predictions) {
        cv::Mat crop;
        Utils::crop(image, crop, prediction.box);
        results.emplace_back(crop);
    }
    return results;
}

void YOLODetection::drawPredictions(mat_vec &images, const std::vector<prediction_vec> &predictionsList, float maskAlpha) const
{
    YOLOInference::drawPredictions(images, predictionsList, maskAlpha);
}

void YOLODetection::drawPredictionsMask(mat_vec &images, const std::vector<prediction_vec> &predictionsList, float maskAlpha) const
{
    YOLOInference::drawPredictionsMask(images, predictionsList, maskAlpha);
}

std::vector<mat_vec> YOLODetection::cropPredictions(const mat_vec &images, const std::vector<prediction_vec> &predictionsList) const
{
    return YOLOInference::cropPredictions(images, predictionsList);
}

std::vector<prediction_vec> YOLODetection::postprocess(const std::vector<cv::Mat> &originalImages,
